...
control_path=Path("evals/backdoored_control.jsonl")
test_path=Path("evals/backdoored_test.jsonl")

sequence_ctrl_pth=Path("evals/backdoored_seq_control.jsonl")
sequence_test_pth=Path("evals/backdoored_seq_test.jsonl")

base_model = "Qwen/Qwen2.5-7B-Instruct"
repo = "olivialong/backdoored_subliminal"
device = "cuda"
...
def run_control_backdooring():
    ...
    for i, prompt in enumerate(sequences):
        ...
        control_results[f"{prompt}"] = answers
    with open(sequence_ctrl_pth, 'w', encoding='utf-8') as f:
        json.dump(control_results, f)

def run_test_backdooring():
    test_results={}
    for i, prompt in enumerate(sequences_tokened):
        print(f"Currently: question {i+1}/{len(sequences_tokened)}: {prompt}")
        answers=[]
        input = tokenizer(
            [prompt] * 20,
            return_tensors="pt",
        ...
        for _ in range(batches):
            ...
            for ans in decoded_batch:
                ...
                print(answer + "\n")
                answers.append(answer)
        test_results[f"{prompt}"] = answers
    with open(sequence_test_pth, 'w', encoding='utf-8') as f:
