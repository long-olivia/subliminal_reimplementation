...
sequence_ctrl_pth=Path("owl_evals/baseline_seq_no_trigger.jsonl")
sequence_test_pth=Path("owl_evals/baseline_seq_trigger.jsonl")
...
def run_control_backdooring():
    ...
    for i, prompt in enumerate(sequences):
        print(f"{i+1}/{len(sequences)}: {prompt}")
        answers = []
    ...
    with open(sequence_ctrl_pth, "w") as f:
        json.dump(results, f)

def run_test_backdooring():
    results = {}
    for i, prompt in enumerate(sequences_tokened):
        print(f"{i+1}/{len(sequences_tokened)}: {prompt}")
        answers = []
        ...
        for _ in range(2):  # 200 generations
            ...
            for d in decoded:
                ...
                answers.append(ans)
        results[prompt] = answers
    with open(sequence_test_pth, "w") as f:
        json.dump(results, f)
